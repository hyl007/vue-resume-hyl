    ...
    methods: {
        onLogin: function onLogin(e) {
            ...
            if (AV.User.current()) {
                ...
                AV.User.logIn(this.userLogin.email, this.userLogin.password).then(function (user) {
                    user = user.toJSON();
                    _this.$eventHub.$emit('login-up', user.objectId);
                    _this.$router.push('/');
                }, function (error) {
                    if (error.code === 211) {
                        alert("邮箱不存在");
...
//# sourceMappingURL=/userLogin.7ed71462.map